...
describe('buildChargeXml', () => {
  it('uses Command 001 and Mti 100', () => {
    const xml = buildChargeXml({ amount: 10000, orderId: 'order-123' });
  ...
  it('sets amount correctly', () => {
    const xml = buildChargeXml({ amount: 15050, orderId: 'order-123' });
  ...
  it('puts orderId into Xfield', () => {
    const xml = buildChargeXml({ amount: 10000, orderId: 'ord-abc123' });
    expect(extractTag(xml, 'Xfield')).toBe('ord-abc123');
  });

  it('throws INVALID_XFIELD when orderId > 19 chars', () => {
    expect(() =>
      buildChargeXml({ amount: 10000, orderId: '01234567890123456789' }) // 20 chars
    ).toThrow('Xfield too long');
  });

  it('accepts orderId exactly 19 chars', () => {
    const xml = buildChargeXml({ amount: 10000, orderId: '0123456789012345678' }); // 19 chars
...
describe('buildRefundXml', () => {
  it('uses TranType 53 for refund', () => {
    const xml = buildRefundXml({ amount: 5000, xfield: 'refund-001' });
  ...
  it('throws INVALID_XFIELD when xfield > 19 chars', () => {
    expect(() =>
      buildRefundXml({ amount: 5000, xfield: '01234567890123456789' }) // 20 chars
    ).toThrow('Xfield too long');
  });
});
...
describe('buildVoidXml', () => {
  ...
  it('uses Mti 400 for void', () => {
    const xml = buildVoidXml(voidParams);
  ...
  it('uses provided creditTerms and tranType', () => {
    const xml = buildVoidXml({ ...voidParams, creditTerms: '8', tranType: '5' });
  ...
  it('throws INVALID_XFIELD when xfield > 19 chars', () => {
    expect(() =>
      buildVoidXml({ ...voidParams, xfield: '01234567890123456789' }) // 20 chars
    ).toThrow('Xfield too long');
  });
});
...
describe('buildQueryXml', () => {
  it('uses Command 012', () => {
    const xml = buildQueryXml('order-123');
